...
class ArchlichKelthuzad(Minion):
    ...
    def destroy_sides(self):
        if (my_pos := self.army.index(self)) > 0 and MinionClass.Undead in self.army[my_pos - 1].classes:
            new_minion = self.destroy(my_pos - 1)
            if len(self.army) < 7:
                self.army.add(new_minion, self.army.index(self))
        if self.triplet and (my_pos := self.army.index(self)) < len(self.army) - 1 and MinionClass.Undead in self.army[my_pos + 1].classes:
            new_minion = self.destroy(my_pos + 1)
            if len(self.army) < 7:
